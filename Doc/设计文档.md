...
sequenceDiagram
    ...
    Service->>API: GET /api/v1/auth/current-user
    API-->>Service: 当前用户信息
    Service->>Store: 同步 username / nickname / avatar / bio 等
    Profile-->>Profile: 展示用户资料

    Profile->>Edit: 点击编辑资料
    Edit->>Service: getProfile()
    Service->>API: GET /api/v1/auth/current-user
    API-->>Edit: 填充原有资料

    Edit->>Service: updateProfile(profile)
    Service->>API: PUT /api/v1/users/me/profile
    API-->>Service: 返回更新后的用户资料
    Service->>Store: 更新全局用户状态
    Edit-->>Profile: 返回个人主页并刷新展示
```
...
sequenceDiagram
    participant User as 用户
    participant Privacy as PrivacySettingPage
    participant Service as PrivacyService
    participant API as 后端隐私接口

    Privacy->>Service: getMyPrivacy(username)
    Service->>API: GET /api/v1/users/{username}/privacy
    API-->>Privacy: 返回当前隐私配置
    Privacy-->>User: 显示开关状态

    User->>Privacy: 切换某项隐私开关
    Privacy->>Privacy: 判断是否为真实状态变化
    Privacy->>Service: updateMyPrivacy(username, privacy)
    Service->>API: PUT /api/v1/users/{username}/privacy
    API-->>Privacy: 返回保存后的隐私配置
    Privacy-->>User: 显示设置已更新
```
